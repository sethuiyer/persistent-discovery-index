...
> **Deep dive — the math behind PDI, and why modern agent benchmarks lie:**
> [watch the video](https://www.youtube.com/watch?v=tkPmvU6x_bc) ·
> [read the essay](https://gist.github.com/shunyabarlabs/ec39c0ad4ee105d283a914b7b0a99aed)
> (both also in-repo as [`MATH.md`](MATH.md))

**Where the value is:** [`CEO.md`](CEO.md) is the commercial read,
[`usecases/`](usecases/README.md) holds six concrete applications with their proof
status and pilot plans, and [`MARKET_SURVEY.md`](MARKET_SURVEY.md) surveys the
closest tools and where PDI's remaining distinctiveness sits.
...
Ordered by topic, not by version. Every entry names its warrant; the full ledger
with search bounds is [`SPINE.md`](SPINE.md) §0, and the long-form derivation is
[`MATH.md`](MATH.md).
...
| A `T`-stable fibre exists (4096 states → 4–40 blocks) | computed | [the fibre](#constructing-the-fibre-the-transport-stable-quotient-v080) |
| Invertibility is **earned** on the recurrent core, not posited | proved | [Prime fields](#prime-fields-where-invertibility-is-free-and-where-it-isnt-v090) |
| The survival square commutes on PDI's axis (`Ω ≡ 0`), fails on the inclusion axis | proved | [commuting square](#the-commuting-square--and-why-it-is-the-wrong-question-for-this-axis-v0120) |
...
| A controlled task corpus — repeated tasks per agent | **blocker** | [Roadmap](#roadmap) |
...
- **Thirty-two self-checking suites**, including closed-form toy validation, the
  ...
  (`test_ingest_cli.py`), the input-warrant matrix (`test_capabilities.py`),
  independent outcome evaluation (`evaluators.py`, `test_evaluators.py`), the
  frozen decision statistic (`o4b_stat.py`, `test_o4b_stat.py`), the cost-variation
  statistic (`o4b_cost.py`, `test_o4b_cost.py`), and the runner trace-persistence
  gate (`o4b_run.py`, `test_o4b_run.py`).
...
The whole chain, in one narrative: why the tower is forced, why the two ledgers
cannot be merged, and exactly which claims are proved, computed, or open.
Claim-by-claim status is the ledger in [`SPINE.md`](SPINE.md) §0.

**Read it in full: [`MATH.md`](MATH.md).** The same essay is also on video
([The Hidden Cost of AI Discovery](https://www.youtube.com/watch?v=tkPmvU6x_bc))
and published as a blog post
([The Persistent Discovery Index](https://gist.github.com/shunyabarlabs/ec39c0ad4ee105d283a914b7b0a99aed)).
